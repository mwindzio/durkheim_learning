...
to setup
  ...
  create-persons 0 [
    ...
    ask persons [if color = green [
      ifelse random-float 100 < 60 ;; 60 % Altruists in green group
      [set altruist 1]
      [set altruist 0]
      set transmiss-p 0]
      ]
    ...
    ask persons [if color = red [
      ifelse random-float 100 < 40 ;; 40 % Altruists in red group
      [set altruist 1]
      [set altruist 0]
      set transmiss-p 0]
      ]
    ...
    ask persons [if color = cyan [
      ifelse random-float 100 < 40 ;; 40 % Altruists in cyan group
      [set altruist 1]
      [set altruist 0]
...
to move  ;; person procedure
  ...
  ifelse spatial_prox
  [
  ifelse altruist = 1 and random-float 100 < spatial_prob [setxy (0 + random-float 15)  (0 + random-float 15)] [move-to-empty] ;; here spatial preference => move x% somewhere to the upper left !
  ]
  [move-to-empty]
end
...
;; Rather "by-product mutualism" or "selfish teamwork" (Dugatkin 1999: 21, 108 )
...
@#$#@#$#@
<experiments>
  <experiment name="Altruism_SPATIAL" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>share-altruists</metric>
    <enumeratedValueSet variable="spatial_prob">
      <value value="0"/>
      <value value="10"/>
      <value value="20"/>
    </enumeratedValueSet>
